...
function PortfolioEditor() {
  ...
  useEffect(() => {
    const nav = mobileStepNavRef.current;
    const currentStep = nav?.querySelector<HTMLElement>(".current");
    if (!nav || !currentStep || !window.matchMedia("(max-width: 680px)").matches) return;
    const left = currentStep.offsetLeft - (nav.clientWidth - currentStep.offsetWidth) / 2;
  ...
  const publish = async () => {
    ...
    if (!validWhatsapp(data.whatsapp)) {
      setPublishError("Agrega un número de WhatsApp válido con código de país antes de publicar.");
      return;
    }
...
function whatsappDigits(value: string) { return value.replace(/\D/g, ""); }
function validWhatsapp(value: string) { return /^\d{8,15}$/.test(whatsappDigits(value)); }
function whatsappLink(value: string) { return `https://wa.me/${whatsappDigits(value)}`; }
...
export function WebsitePortfolio({ data, media, brands, schema, expanded = false }: { data: Portfolio; media: Media[]; brands: BrandAsset[]; schema: TemplateSchema; expanded?: boolean }) {
  ...
  return <div className={`websitePortfolio website-${webTemplate} font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--site-accent": portfolioAccent(data) } as CSSProperties}>
    ...
    <footer className="siteFooter">{hasText(data.name) && <strong>{data.name} <Icon glyph="✦" /></strong>}{hasText(data.location) && <span>{data.location}</span>}</footer>
  </div>;
}
...
function MediaCard({ item, label, index }: { item: Media | null; label: string; index: number }) { const framed = item?.type === "video" && item.framed; const video = item?.type === "video" ? <video src={item.url} poster={item.previewUrl} muted playsInline controls preload="metadata" onPlay={enterMobileFullscreen} /> : null; return <article className={`deckMedia media-${index} ${framed ? "deviceFramed" : item?.type === "video" ? "videoPlain" : ""}`}>{item ? item.type === "video" ? <>{framed ? <PhoneFrame>{video}</PhoneFrame> : video}<div className="videoSocials">{item.instagram && <a href={item.instagram} target="_blank" rel="noreferrer" aria-label="Ver en Instagram" data-analytics-target="instagram">IG</a>}{item.tiktok && <a href={item.tiktok} target="_blank" rel="noreferrer" aria-label="Ver en TikTok" data-analytics-target="tiktok">TK</a>}</div></> : <img src={item.url} alt={`Pieza UGC de ${label}`} loading="lazy" /> : <div className="mediaPlaceholder"><span>{label}</span><b>{String(index + 1).padStart(2, "0")}</b><i><Icon glyph="▶" /></i></div>}</article>; }
