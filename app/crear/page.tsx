...
const websiteOptions = [
  { name: "Muse Editorial", note: "Magazine cálido · lujo relajado", mode: "muse", font: "Editorial", defaultFont: "editorial" },
  { name: "Creator Studio", note: "Digital nocturno · energía modular", mode: "creator", font: "Moderna", defaultFont: "modern" },
  { name: "Aura Grid", note: "Pop romántico · curvas en movimiento", mode: "aura", font: "Romántica", defaultFont: "romantic" },
  { name: "Noir Atelier", note: "Cine oscuro · impacto premium", mode: "noir", font: "Magazine", defaultFont: "magazine" },
  { name: "Sorbet Studio", note: "Durazno pastel · bloques editoriales", mode: "sorbet", font: "Magazine", defaultFont: "magazine" },
  { name: "Lavender Cloud", note: "Lavanda suave · aire minimalista", mode: "lavender", font: "Editorial", defaultFont: "editorial" },
  { name: "Mint Picnic", note: "Menta y mantequilla · energía orgánica", mode: "mint", font: "Romántica", defaultFont: "romantic" },
  { name: "Electric Pulse", note: "Neón eléctrico · diversión digital", mode: "electric", font: "Moderna", defaultFont: "modern" },
];
...
const initial: Portfolio = {
  ...
  location: "Bogotá, Colombia", niches: ["Beauty", "Lifestyle", "Travel"], format: "website", webTemplate: "muse", template: "gallery", fontStyle: "editorial", accent: "#c15f7a", portfolioCategories: categories,
...
function PortfolioEditor() {
  ...
  const schema = templateSchemas[data.format === "website" ? data.webTemplate : data.template] ?? templateSchemas.muse;
...
export function WebsitePortfolio({ data, media, brands, schema, expanded = false }: { data: Portfolio; media: Media[]; brands: BrandAsset[]; schema: TemplateSchema; expanded?: boolean }) {
  ...
  return <div className={`websitePortfolio website-${data.webTemplate} font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--site-accent": data.accent } as CSSProperties}>
    ...
    {groups.length > 0 && <section className="siteWork webSection" id="web-work"><div className="webSectionTitle"><small>02 · TRABAJO SELECCIONADO</small><h2>{data.campaignTitle}</h2></div>{groups.map((group) => <article className="webProjectGroup" key={group.category}><header><h3>{group.category}</h3><span>{String(group.items.length).padStart(2, "0")} / {group.category === "Fotografía" ? schema.photoLimit : schema.categoryLimit} piezas</span></header><div style={{ "--work-count": Math.min(group.items.length, schema.categoryLimit) } as CSSProperties}>{group.items.map((item, index) => <MediaCard key={item.id} item={item} label={item.category} index={index} />)}</div></article>)}</section>}
