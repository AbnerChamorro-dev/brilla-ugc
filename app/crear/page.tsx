...
function PortfolioEditor() {
  ...
  return <main className="builderApp">
    ...
    <div className="builderGrid">
      ...
      <aside className={`livePreview ${mobilePreviewOpen ? "" : "mobilePreviewCollapsed"}`}><div className="previewHeader"><div><span>VISTA PREVIA</span><strong>{data.format === "website" ? "Página web · cambios en vivo" : "Presentación horizontal · cambios en vivo"}</strong></div><small>{data.format === "website" ? "Scroll ↓" : "Desliza →"}</small><button className="mobilePreviewToggle" type="button" aria-expanded={mobilePreviewOpen} onClick={() => setMobilePreviewOpen((current) => !current)}>{mobilePreviewOpen ? "Ocultar" : "Mostrar"}<span aria-hidden="true">{mobilePreviewOpen ? "↑" : "↓"}</span></button></div>{data.format === "website" ? <WebsitePortfolio data={data} media={media} brands={brands} schema={schema} /> : <PortfolioDeck data={data} media={media} brands={brands} schema={schema} />}</aside>
    </div>
