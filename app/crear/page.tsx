  ...
  return <main className="builderApp">
    ...
    <div className="builderGrid">
      ...
      <section className="builderFormArea">
        ...
        {step === 0 && <div className="formPanel"><div className="visualSettings"><div className="languageChoice"><span>Idioma del portafolio</span><p>Los títulos y botones de la plantilla se mostrarán en este idioma.</p><div><button className={data.language === "es" ? "selected" : ""} onClick={() => update("language", "es")}>Español</button><button className={data.language === "en" ? "selected" : ""} onClick={() => update("language", "en")}>English</button></div></div><div className="choiceField colorChoice"><span>Color de la plantilla</span><p>El cambio aparece al instante en la vista previa.</p><div>{colors.map((color) => <button key={color} aria-label={`Elegir ${color}`} className={data.accent === color ? "selected" : ""} style={{ background: color }} onClick={() => update("accent", color)} />)}<label><input aria-label="Color personalizado" type="color" value={data.accent || "#6d4dff"} onChange={(e) => update("accent", e.target.value)} />＋</label></div></div></div><div className="choiceField templateFamily"><span>Plantillas de página web <small>{websiteOptions.length} estilos profesionales</small></span><p>Sitios verticales con navegación, secciones y transiciones suaves.</p><div className="themeCards webThemeCards">{websiteOptions.map((item) => <Theme key={item.mode} {...item} accent={data.accent || "#6d4dff"} current={data.format === "website" ? data.webTemplate : ""} choose={(mode, fontStyle) => setData((current) => ({ ...current, format: "website", webTemplate: mode, fontStyle }))} />)}</div></div><div className="templateDivider"><span>O ELIGE UNA EXPERIENCIA PRESENTACIONAL</span></div><div className="choiceField templateFamily"><span>Plantillas presentacionales <small>7 estilos</small></span><p>Láminas horizontales con navegación por gestos y flechas.</p><div className="themeCards">{templateOptions.map((item) => <Theme key={item.mode} {...item} accent={data.accent || "#6d4dff"} current={data.format === "presentation" ? data.template : ""} choose={(mode, fontStyle) => setData((current) => ({ ...current, format: "presentation", template: mode, fontStyle }))} />)}</div></div><div className="choiceField"><span>Tipo de letra</span><div className="fontCards">{fontOptions.map((font) => <button key={font.value} className={data.fontStyle === font.value ? "selected" : ""} onClick={() => update("fontStyle", font.value)}><b>{font.sample}</b><span>{font.name}</span><small>{font.note}</small></button>)}</div></div></div>}
        {step === 2 && <div className="formPanel"><div className="schemaSummary"><span>✦</span><p><strong>{schema.id.replace("gallery", "Gallery").replace("studio", "Studio Luv").replace("scrapbook", "Scrapbook").replace("art", "Art Director").replace("blue", "Blue OS").replace("whimsy", "Whimsy").replace("sage", "Sage Journal").replace("muse", "Muse Editorial").replace("creator", "Creator Studio").replace("aura", "Aura Grid").replace("noir", "Noir Atelier").replace("sorbet", "Sorbet Studio").replace("lavender", "Lavender Cloud").replace("mint", "Mint Picnic").replace("electric", "Electric Pulse").replace("pop", "Sunny Pop").replace("retro", "Retro Zine").replace("chic", "Éditorial Chic").replace("bold", "Neo Brutal")}</strong><small>{schema.label}. El formulario respeta su composición.</small></p></div><Field label="Título de la sección de trabajos" value={data.campaignTitle} set={(v) => update("campaignTitle", v)} placeholder="Ej. Contenido que convierte" help="Es el encabezado que verá la marca antes de tus videos y fotos. Si lo dejas vacío, no se muestra." /><Choice title="Sectores con los que trabajas" options={clientOptions} selected={data.clientTypes} toggle={(v) => toggle("clientTypes", v)} /><div className="portfolioCategoryPicker"><Choice title="Categorías para organizar tus trabajos" options={categories} selected={data.portfolioCategories} toggle={(v) => toggle("portfolioCategories", v)} /><p>Primero elige una categoría y luego sube sus videos o fotos. Nada está preseleccionado.</p></div>{data.portfolioCategories.length > 0 && <div className="activeCategoryGuide"><span>1</span><div><strong>¿A qué categoría pertenece el archivo?</strong><small>Selecciona una antes de subir. La categoría activa queda resaltada.</small></div></div>}<div className="categoryTabs" role="tablist">{data.portfolioCategories.map((item) => <button key={item} className={category === item ? "active" : ""} onClick={() => setCategory(item)}>{item}<small>{workMedia.filter((m) => m.category === item).length}/{item === "Fotografía" ? schema.photoLimit : schema.categoryLimit}</small></button>)}</div>{data.portfolioCategories.length ? <label className="mediaDrop"><input type="file" accept="video/*,image/*" multiple onChange={upload} /><span>↑</span><strong>Subiendo a: “{data.portfolioCategories.includes(category) ? category : data.portfolioCategories[0]}”</strong><small>Imágenes hasta 10 MB · videos hasta 50 MB · {schema.label}.</small><b>Seleccionar archivos</b></label> : <div className="emptyCategory">Selecciona al menos una categoría para agregar contenido.</div>}{workMedia.length > 0 && <div className="mediaList">{workMedia.map((item) => <article key={item.id} className="mediaRow"><div className="mediaThumb">{item.previewUrl ? <img src={item.previewUrl} alt="Vista previa del contenido" /> : item.type === "video" ? <video src={item.url} muted /> : <img src={item.url} alt="Contenido subido" />}</div><div className="mediaInfo"><strong>{item.name}</strong><small><b className="mediaCategoryBadge">{item.category}</b>{item.type === "video" ? "Video" : "Foto"}</small>{item.type === "video" && <><button className={`frameChoice ${item.framed ? "selected" : ""}`} onClick={() => updateMedia(item.id, "framed", !item.framed)}>{item.framed ? "✓ Con marco de teléfono" : "Sin marco de teléfono"}</button><div className="mediaLinks"><input value={item.instagram} onChange={(e) => updateMedia(item.id, "instagram", e.target.value)} placeholder="Link de Instagram" /><input value={item.tiktok} onChange={(e) => updateMedia(item.id, "tiktok", e.target.value)} placeholder="Link de TikTok" /></div></>}</div><button className="removeMedia" onClick={() => remove(item.id)} aria-label="Eliminar">×</button></article>)}</div>}<div className="brandUpload"><div><strong>Logos de marcas</strong><small>JPG, PNG, WebP o GIF · máximo 10 MB por logo.</small></div><label><input type="file" accept="image/*" multiple onChange={uploadBrands} />＋ Agregar logos</label></div>{brands.length > 0 && <div className="brandList">{brands.map((brand) => <article key={brand.id}><img src={brand.url} alt={brand.name} /><span>{brand.name}</span><button onClick={() => removeBrand(brand.id)} aria-label={`Eliminar ${brand.name}`}>×</button></article>)}</div>}</div>}
...
function Theme({ name, note, mode, font, defaultFont, current, choose, accent }: { name: string; note: string; mode: string; font: string; defaultFont: string; current: string; choose: (mode: string, fontStyle: string) => void; accent: string }) { return <button className={current === mode ? "selected" : ""} style={{ "--theme-accent": accent } as CSSProperties} onClick={() => choose(mode, defaultFont)}><i className={`themePreview ${mode}`}><b>{name}</b><em>Aa</em><u /></i><strong>{name}</strong><small>{note}</small><span>{templateSchemas[mode].label} · {font}</span></button>; }
...
function portfolioText(data: Portfolio, spanish: string, english: string) { return data.language === "en" ? english : spanish; }
...
export function WebsitePortfolio({ data, media, brands, schema, expanded = false }: { data: Portfolio; media: Media[]; brands: BrandAsset[]; schema: TemplateSchema; expanded?: boolean }) {
  ...
  return <div className={`websitePortfolio website-${webTemplate} font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--site-accent": portfolioAccent(data) } as CSSProperties}>
    ...
    {ticker.length > 0 && <div className="webMarquee" aria-hidden><div>{Array.from({ length: 2 }, (_, dup) => ticker.map((word, i) => <span key={`${dup}-${i}`}>{word}<i>✦</i></span>))}</div></div>}
    ...
    {hasAbout && <section className="webAbout webSection" id="web-about"><div className="webSectionTitle"><small>01 — {portfolioText(data, "SOBRE MÍ", "ABOUT ME")}</small><h2>{portfolioText(data, "Historias reales que conectan con tu audiencia.", "Real stories that connect with your audience.")}</h2></div><div className="aboutBody">{hasText(data.bio) && <p>{data.bio}</p>}{aboutTags.length > 0 && <div className="webTags">{aboutTags.map((item) => <span key={item}>{item}</span>)}</div>}{data.contentTypes.length > 0 && <div className="aboutContent"><b>{portfolioText(data, "CONTENIDO QUE CREO", "CONTENT I CREATE")}</b><p>{data.contentTypes.join(" · ")}</p></div>}</div></section>}
    {groups.length > 0 && <section className="siteWork webSection" id="web-work"><div className="webSectionTitle"><small>02 — {portfolioText(data, "PORTAFOLIO", "PORTFOLIO")}</small>{hasText(data.campaignTitle) && <h2>{data.campaignTitle}</h2>}</div>{groups.map((group, gi) => <article className="webProjectGroup" key={group.category}><header><h3><i>{icons[gi % icons.length]}</i>{group.category}</h3><span>{String(group.items.length).padStart(2, "0")} / {group.category === "Fotografía" ? schema.photoLimit : schema.categoryLimit} {portfolioText(data, "PIEZAS", "PIECES")}</span></header><div style={{ "--work-count": Math.min(group.items.length, 4) } as CSSProperties}>{group.items.map((item, index) => <MediaCard key={item.id} item={item} label={item.category} index={index} />)}</div></article>)}</section>}
    ...
    {hasRates && <section className="webRates webSection" id="web-rates"><div className="webSectionTitle"><small>06 — TARIFAS</small><h2>Inversión clara, <em>sin sorpresas</em>.</h2></div><div className="rateBoard">{(hasText(data.videoRate) || data.includes.length > 0) && <article className="rateHero"><small>EL FAVORITO ✦</small><h3>Video UGC</h3>{hasText(data.videoRate) && <b>{data.videoRate}</b>}{data.includes.length > 0 && <ul>{data.includes.map((include) => <li key={include}><i>✓</i>{include}</li>)}</ul>}</article>}{hasAnyText(data.collabRate, data.storyRate, data.storyPackRate, data.usageRate) && <div className="rateGrid">{hasText(data.collabRate) && <article><span>Reel en colaboración</span><b>{data.collabRate}</b></article>}{hasText(data.storyRate) && <article><span>1 historia con CTA</span><b>{data.storyRate}</b></article>}{hasText(data.storyPackRate) && <article><span>Pack de 3 historias</span><b>{data.storyPackRate}</b></article>}{hasText(data.usageRate) && <article><span>Derechos de pauta</span><b>{data.usageRate}</b></article>}</div>}</div></section>}
