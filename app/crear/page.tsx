...
export function WebsitePortfolio({ data, media, brands, schema, expanded = false }: { data: Portfolio; media: Media[]; brands: BrandAsset[]; schema: TemplateSchema; expanded?: boolean }) {
  ...
  const ticker = [...data.niches, hasText(data.location) ? data.location.split(",")[0]?.trim() : "", ...data.contentTypes.slice(0, 3)].filter(Boolean);
  const icons = ["✦", "♡", "☆", "❀", "◎", "✧", "⌂"];
  ...
  return <div className={`websitePortfolio website-${webTemplate} font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--site-accent": data.accent } as CSSProperties}>
    <header className="siteNav">{hasText(firstName) && <strong>{firstName}<i>✦</i></strong>}<nav>{groups.length > 0 && <a href="#web-work">Trabajo</a>}{data.services.length > 0 && <a href="#web-services">Servicios</a>}{hasRates && <a href="#web-rates">Tarifas</a>}{hasText(data.email) && <a className="navContact" href={emailLink(data.email)} data-analytics-target="email">Hablemos ↗</a>}</nav></header>
    <section className="siteHero webSection"><div className="webHeroCopy">{hasText(data.location) && <small><i>✦</i> UGC CREATOR — {data.location}</small>}{hasText(data.name) && <h1>{data.name}</h1>}{hasText(data.title) && <p className="heroRole">{data.title}</p>}{hasText(data.bio) && <p className="heroBio">{data.bio}</p>}{(groups.length > 0 || hasText(data.email)) && <div className="webHeroActions">{groups.length > 0 && <a className="heroCta" href="#web-work">Ver mi trabajo ↓</a>}{hasText(data.email) && <a className="heroGhost" href={emailLink(data.email)} data-analytics-target="email">Escríbeme</a>}</div>}</div>{(portrait || hasText(data.availability)) && <div className="webHeroVisual">{portrait && <MediaCard item={portrait} label="TU FOTO" index={0} />}<span className="heroSticker st1">✦</span><span className="heroSticker st2">☆</span>{hasText(data.availability) && <em className="heroNote">{data.availability}</em>}</div>}</section>
    ...
    {hasAnyText(data.followers, data.monthlyViews, data.womenAudience) || data.niches.length > 0 ? <div className="webProof">{hasText(data.followers) && <span><b>{data.followers}</b>seguidores</span>}{hasText(data.monthlyViews) && <span><b>{data.monthlyViews}</b>vistas / mes</span>}{hasText(data.womenAudience) && <span><b>{data.womenAudience}</b>audiencia femenina</span>}{data.niches.length > 0 && <span><b>{String(data.niches.length).padStart(2, "0")}</b>nichos creativos</span>}</div> : null}
    {hasAbout && <section className="webAbout webSection" id="web-about"><div className="webSectionTitle"><small>01 — SOBRE MÍ</small><h2>Historias reales que <em>conectan</em> con tu audiencia.</h2></div><div className="aboutBody">{hasText(data.bio) && <p>{data.bio}</p>}{aboutTags.length > 0 && <div className="webTags">{aboutTags.map((item) => <span key={item}>{item}</span>)}</div>}{data.contentTypes.length > 0 && <div className="aboutContent"><b>CONTENIDO QUE CREO</b><p>{data.contentTypes.join(" · ")}</p></div>}</div></section>}
    ...
    {hasAudience && <section className="webAudience webSection"><div className="audienceCopy"><small>04 — AUDIENCIA</small><h2>Una comunidad que <em>confía</em> en lo que recomiendo.</h2>{hasAnyText(data.followers, data.monthlyViews, data.womenAudience) && <div className="webNumbers">{hasText(data.followers) && <span><b>{data.followers}</b>seguidores</span>}{hasText(data.monthlyViews) && <span><b>{data.monthlyViews}</b>vistas / mes</span>}{hasText(data.womenAudience) && <span><b>{data.womenAudience}</b>mujeres</span>}</div>}{hasAnyText(data.instagram, data.tiktok) && <div className="audienceSocial">{hasText(data.instagram) && <a href={socialLink("instagram", data.instagram)} target="_blank" rel="noreferrer" data-analytics-target="instagram"><b>IG</b>{data.instagram}</a>}{hasText(data.tiktok) && <a href={socialLink("tiktok", data.tiktok)} target="_blank" rel="noreferrer" data-analytics-target="tiktok"><b>TK</b>{data.tiktok}</a>}</div>}</div>{(hasText(data.womenAudience) || countries.length > 0) && <div className="webAudienceCard">{hasText(data.womenAudience) && <div className="webAudienceRing"><b>{data.womenAudience}</b><span>audiencia femenina</span></div>}{countries.length > 0 && <div className="countryBars">{countries.map((country, index) => <p key={country}><span>{country}</span><i style={{ width: `${Math.max(18, 88 - index * 18)}%` }} /></p>)}</div>}</div>}</section>}
    {data.services.length > 0 && <section className="webServices webSection" id="web-services"><div className="webSectionTitle"><small>05 — SERVICIOS</small><h2>Todo lo que puedo <em>crear</em> para tu marca.</h2></div><div className="serviceCards">{data.services.map((service, index) => <article key={service}><span>{String(index + 1).padStart(2, "0")}</span><h3>{service}</h3><b>{icons[index % icons.length]}</b></article>)}</div></section>}
...
function IntroSlide({ data, active, index, portrait }: SlideProps) { return <section className={`deckSlide intro ${active === index ? "isActive" : ""}`}><div className="introCopy"><span>CREADORA DE CONTENIDO UGC</span>{hasText(data.title) && <h1>{data.title}</h1>}{hasText(data.bio) && <p>{data.bio}</p>}{hasText(data.location) && <small>{data.location}</small>}</div>{portrait && <MediaCard item={portrait} label="TU RETRATO" index={0} />}</section>; }
