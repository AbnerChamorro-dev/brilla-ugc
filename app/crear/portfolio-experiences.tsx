...
export function CreatorFeedPortfolio({ data, media, brands, schema, expanded }: ExperienceProps) {
  ...
  return <div className={`creatorFeed font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--site-accent": data.accent, "--experience-accent": data.accent } as CSSProperties}>
    <header className="feedNav"><a className="feedIdentity" href="#feed-top"><span className="feedAvatar">{initials(data.name)}</span><span>{hasText(data.name) && <strong>{data.name}</strong>}{hasText(data.location) && <small>UGC creator · {data.location}</small>}</span></a><nav>{work.length > 0 && <a href="#feed-work">Contenido</a>}{hasText(data.bio) && <a href="#feed-story">Sobre mí</a>}{data.services.length > 0 && <a href="#feed-services">Servicios</a>}</nav>{hasText(data.email) && <a className="feedNavCta" href={emailLink(data.email)} data-analytics-target="email">Trabajemos ↗</a>}</header>
    ...
    {(hasText(data.bio) || data.contentTypes.length > 0 || hasAnyText(data.followers, data.monthlyViews, data.womenAudience)) && <section className="feedStory feedSection" id="feed-story"><div className="feedStoryCopy"><span className="feedStoryLabel">MI MANERA DE CREAR</span><h2>No hago anuncios. <em>Creo recomendaciones.</em></h2>{hasText(data.bio) && <p>{data.bio}</p>}</div>{(data.contentTypes.length > 0 || hasAnyText(data.followers, data.monthlyViews, data.womenAudience)) && <div className="feedStoryPanel">{data.contentTypes.length > 0 && <><small>FORMATOS QUE DISFRUTO</small><div className="feedFormats">{data.contentTypes.map((type) => <span key={type}>{type}</span>)}</div></>}{hasAnyText(data.followers, data.monthlyViews, data.womenAudience) && <div className="feedMicroMetrics">{hasText(data.followers) && <span><b>{data.followers}</b>seguidores</span>}{hasText(data.monthlyViews) && <span><b>{data.monthlyViews}</b>vistas / mes</span>}{hasText(data.womenAudience) && <span><b>{data.womenAudience}</b>mujeres</span>}</div>}</div>}</section>}
    ...
    {(data.services.length > 0 || hasText(data.womenAudience) || countries.length > 0) && <section className="feedCollab feedSection" id="feed-services">{data.services.length > 0 && <div><small className="feedKicker">TRABAJEMOS JUNTAS</small><h2>Un menú de contenido hecho para tu marca.</h2><div className="feedServiceList">{data.services.map((service, index) => <div className="feedServiceRow" key={service}><span>{String(index + 1).padStart(2, "0")}</span><strong>{service}</strong><small>{servicePrice(data, service)}</small></div>)}</div></div>}{(hasText(data.womenAudience) || countries.length > 0) && <aside className="feedAudienceCard"><small>MI COMUNIDAD</small><h3>Personas reales conectando con historias reales.</h3>{hasText(data.womenAudience) && <div className="feedAudienceHero"><b>{data.womenAudience}</b><span>audiencia femenina</span></div>}{countries.length > 0 && <div className="feedCountryList">{countries.map((country) => <p key={country}><span>{country}</span></p>)}</div>}</aside>}</section>}
    {hasAnyText(data.availability, data.email) && <section className="feedContact"><small className="feedKicker">¿TIENES UNA IDEA?</small><h2>Hagamos contenido que no parezca publicidad.</h2>{hasText(data.availability) && <p>{data.availability}</p>}{hasText(data.email) && <a className="feedContactButton" href={emailLink(data.email)} data-analytics-target="email">Empecemos un proyecto <span>↗</span></a>}</section>}
...
export function PostcardJournalPortfolio({ data, media, brands, schema, expanded }: ExperienceProps) {
  ...
  return <div className={`portfolioExperiencePage postcardJournal font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--experience-accent": data.accent } as CSSProperties}>
    ...
    {hasAnyText(data.email, data.instagram, data.whatsapp, data.location) && <section className="pcContact"><span>PRÓXIMO DESTINO</span><h2>Tu marca puede ser la siguiente historia.</h2>{hasText(data.email) && <a href={emailLink(data.email)} data-analytics-target="email">Reservemos la fecha <b>→</b></a>}<footer>{hasText(data.instagram) && <a href={socialLink("instagram", data.instagram)} data-analytics-target="instagram">{data.instagram}</a>}{hasText(data.whatsapp) && <a href={whatsappLink(data.whatsapp)} data-analytics-target="whatsapp">{data.whatsapp}</a>}{hasText(data.location) && <b>{data.location}</b>}</footer></section>}
  </div>;
}
...
export function CampaignStoriesPortfolio({ data, media, brands, schema, expanded }: ExperienceProps) {
  ...
  return <div className={`portfolioExperiencePage campaignStories font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--experience-accent": data.accent } as CSSProperties}>
    <header className="csNav"><a href="#cs-top">{hasText(data.name) && <strong>{data.name}</strong>}<span>Creative portfolio</span></a><nav aria-label="Secciones">{cases.length > 0 && <a href="#cs-cases">Casos</a>}{data.services.length > 0 && <a href="#cs-services">Servicios</a>}{hasText(data.email) && <a className="csContactLink" href={emailLink(data.email)} data-analytics-target="email">Contacto ↗</a>}</nav></header>
    <section className="csHero" id="cs-top"><div className="csHeroCopy"><span className="csEdition">UGC · CREATIVE CASE FILES</span><h1>Ideas que se sienten.<br /><em>Estrategia que funciona.</em></h1>{hasText(data.bio) && <p>{data.bio}</p>}{hasAnyText(data.followers, data.monthlyViews, data.location) && <div className="csHeroMeta">{hasText(data.followers) && <span><b>{data.followers}</b> seguidores</span>}{hasText(data.monthlyViews) && <span><b>{data.monthlyViews}</b> vistas / mes</span>}{hasText(data.location) && <span><b>{data.location}</b> base creativa</span>}</div>}</div><div className="csPortrait"><ExperienceMedia item={portrait} label="Retrato de creadora" priority /><span>ESTRATEGIA<br />+ PRODUCCIÓN</span></div></section>
    {cases.length > 0 && <section className="csCases" id="cs-cases"><header className="csSectionIntro"><span>CASOS SELECCIONADOS</span>{hasText(data.campaignTitle) && <h2>{data.campaignTitle}</h2>}<p>Cada pieza parte de una idea clara y termina lista para conectar, convertir o construir confianza.</p></header>{cases.map((group, index) => {
      ...
      return <article className="csCase" key={group.category}><div className="csCaseNumber">CASE {String(index + 1).padStart(2, "0")}</div><div className="csCaseCopy"><small>{story.client || group.category}</small><h3>{group.category}</h3>{hasAnyText(brief, hook, story.result) && <dl>{hasText(brief) && <div><dt>EL BRIEF</dt><dd>{brief}</dd></div>}{hasText(hook) && <div className="csHook"><dt>EL HOOK</dt><dd>“{hook}”</dd></div>}{hasText(story.result) && <div><dt>EL RESULTADO</dt><dd>{story.result}</dd></div>}</dl>}{hasText(story.testimonial) && <blockquote>“{story.testimonial}”{hasText(story.client) && <cite>— {story.client}</cite>}</blockquote>}</div><div className={`csCaseMedia count-${mediaSlots.length}`}>{mediaSlots.map((item, mediaIndex) => <ExperienceMedia key={item.id ?? `${group.category}-${mediaIndex}`} item={item} label={group.category} />)}</div></article>;
    })}</section>}
    ...
    {(data.includes.length > 0 || data.services.length > 0) && <section className="csServices" id="cs-services"><div><span>DEL CONCEPTO A LA ENTREGA</span><h2>Contenido con intención en cada etapa.</h2>{data.includes.length > 0 && <p>{data.includes.join(" · ")}</p>}</div>{data.services.length > 0 && <div className="csServiceList">{data.services.map((service, index) => <article key={service}><span>{String(index + 1).padStart(2, "0")}</span><strong>{service}</strong><small>{servicePrice(data, service)}</small></article>)}</div>}</section>}
    {hasAnyText(data.email, data.instagram, data.whatsapp) && <section className="csContact"><span>PRÓXIMO CASO</span><h2>Hagamos una campaña que merezca ser contada.</h2>{hasText(data.email) && <a href={emailLink(data.email)} data-analytics-target="email">Cuéntame sobre tu marca <b>↗</b></a>}{hasAnyText(data.instagram, data.whatsapp) && <div>{hasText(data.instagram) && <a href={socialLink("instagram", data.instagram)} target="_blank" rel="noreferrer" data-analytics-target="instagram">{data.instagram}</a>}{hasText(data.whatsapp) && <a href={whatsappLink(data.whatsapp)} target="_blank" rel="noreferrer" data-analytics-target="whatsapp">{data.whatsapp}</a>}</div>}</section>}
  </div>;
}
...
export function PersonalScrapbookPortfolio({ data, media, brands, schema, expanded }: ExperienceProps) {
  ...
  return <div className={`portfolioExperiencePage personalScrapbook font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--experience-accent": palette, "--chosen-accent": data.accent } as CSSProperties}>
    ...
    {(hasAnyText(data.availability, data.location, data.creativeDiary, data.bio) || process.length > 0) && <section className="psDiary" id="ps-diary">{hasAnyText(data.availability, data.location) && <aside><span>NOTA PARA LAS MARCAS</span>{hasText(data.availability) && <b>{data.availability}</b>}{hasText(data.location) && <small>{data.location}</small>}</aside>}<div><small>ASÍ CREO CONTENIDO</small><h2>Mi proceso comienza con una emoción, no con un anuncio.</h2>{hasAnyText(data.creativeDiary, data.bio) && <p>{data.creativeDiary || data.bio}</p>}{process.length > 0 && <div className="psProcess">{process.map((item, index) => <span key={item}><b>0{index + 1}</b>{item}</span>)}</div>}</div></section>}
    ...
    {(data.services.length > 0 || hasAnyText(data.followers, data.monthlyViews) || data.contentTypes.length > 0) && <section className="psNotes">{data.services.length > 0 && <div><span>LO QUE PUEDO CREAR</span><h2>Ideas listas para cobrar vida.</h2>{data.services.map((service) => <p key={service}>✦ {service}</p>)}</div>}{hasAnyText(data.followers, data.monthlyViews) && <aside><small>MI COMUNIDAD</small>{hasText(data.followers) && <><strong>{data.followers}</strong><span>personas siguiendo la historia</span></>}{hasText(data.monthlyViews) && <b>{data.monthlyViews} vistas / mes</b>}</aside>}{data.contentTypes.length > 0 && <div className="psFormats">{data.contentTypes.map((type) => <span key={type}>{type}</span>)}</div>}</section>}
    ...
    {hasAnyText(data.email, data.instagram, data.tiktok) && <section className="psContact"><span>UNA ÚLTIMA NOTITA</span><h2>¿Creamos algo juntas?</h2><p>Cuéntame qué imaginas para tu marca. Yo pongo la cámara, las ideas y el cuidado en cada detalle.</p>{hasText(data.email) && <a href={emailLink(data.email)} data-analytics-target="email">Sí, hablemos <b>♡</b></a>}{hasAnyText(data.instagram, data.tiktok) && <div>{hasText(data.instagram) && <a href={socialLink("instagram", data.instagram)} target="_blank" rel="noreferrer" data-analytics-target="instagram">Instagram · {data.instagram}</a>}{hasText(data.tiktok) && <a href={socialLink("tiktok", data.tiktok)} target="_blank" rel="noreferrer" data-analytics-target="tiktok">TikTok · {data.tiktok}</a>}</div>}</section>}
  </div>;
}
...
export function ShowreelFirstPortfolio({ data, media, brands, schema, expanded }: ExperienceProps) {
  ...
  return <div className={`portfolioExperiencePage showreelFirst font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--experience-accent": data.accent } as CSSProperties}>
    {hasText(data.email) && <a className="srFloatingContact" href={emailLink(data.email)} data-analytics-target="email">Trabajemos <span>↗</span></a>}
    ...
    {(hasAnyText(data.followers, data.monthlyViews, data.womenAudience) || brands.length > 0) && <section className="srProof">{hasText(data.followers) && <span><b>{data.followers}</b> seguidores</span>}{hasText(data.monthlyViews) && <span><b>{data.monthlyViews}</b> vistas mensuales</span>}{hasText(data.womenAudience) && <span><b>{data.womenAudience}</b> audiencia femenina</span>}{brands.length > 0 && <span><b>{brands.length}</b> marcas</span>}</section>}
    ...
    {(data.services.length > 0 || data.includes.length > 0) && <section className="srServices" id="sr-services"><header><small>SERVICIOS + TARIFAS</small><h2>¿Qué creamos?</h2></header>{data.services.length > 0 && <div>{data.services.map((service, index) => <article key={service}><span>{String(index + 1).padStart(2, "0")}</span><h3>{service}</h3><b>{servicePrice(data, service)}</b></article>)}</div>}{data.includes.length > 0 && <p>{data.includes.join(" · ")}</p>}</section>}
    {hasAnyText(data.email, data.location, data.instagram, data.tiktok) && <section className="srContact"><span>FIN DEL REEL · INICIO DE ALGO NUEVO</span><h2>Tu producto.<br />Mi próxima historia.</h2>{hasText(data.email) && <a href={emailLink(data.email)} data-analytics-target="email">{data.email} ↗</a>}<footer>{hasText(data.location) && <span>{data.location}</span>}{hasText(data.instagram) && <a href={socialLink("instagram", data.instagram)} target="_blank" rel="noreferrer" data-analytics-target="instagram">{data.instagram}</a>}{hasText(data.tiktok) && <a href={socialLink("tiktok", data.tiktok)} target="_blank" rel="noreferrer" data-analytics-target="tiktok">{data.tiktok}</a>}</footer></section>}
  </div>;
}
...
export function TalentProfilePortfolio({ data, media, brands, schema, expanded }: ExperienceProps) {
  ...
  return <div className={`portfolioExperiencePage talentProfile font-${data.fontStyle} ${expanded ? "expanded" : "compact"}`} style={{ "--experience-accent": data.accent } as CSSProperties}>
    ...
    <section className="tpHero" id="tp-top"><div className="tpPortrait"><ExperienceMedia item={portrait} label="Retrato profesional" priority /><span>{initials(data.name)}</span></div><div className="tpIntro">{hasText(data.location) && <span>CREATOR PROFILE · {data.location}</span>}{hasText(data.name) && <h1>{data.name}</h1>}{hasText(data.title) && <p className="tpRole">{data.title}</p>}{hasText(data.bio) && <p className="tpBio">{data.bio}</p>}{hasAnyText(data.location, data.languages, data.availability) && <div className="tpFacts">{hasText(data.location) && <span><small>BASE</small><b>{data.location}</b></span>}{hasText(data.languages) && <span><small>IDIOMAS</small><b>{data.languages}</b></span>}{hasText(data.availability) && <span><small>ESTADO</small><b>{data.availability}</b></span>}</div>}{data.niches.length > 0 && <div className="tpTags">{data.niches.map((niche) => <span key={niche}>{niche}</span>)}</div>}{hasAnyText(data.instagram, data.tiktok) && <div className="tpSocial">{hasText(data.instagram) && <a href={socialLink("instagram", data.instagram)} target="_blank" rel="noreferrer" data-analytics-target="instagram">Instagram ↗</a>}{hasText(data.tiktok) && <a href={socialLink("tiktok", data.tiktok)} target="_blank" rel="noreferrer" data-analytics-target="tiktok">TikTok ↗</a>}</div>}</div></section>
    ...
    {(hasAnyText(data.followers, data.monthlyViews, data.womenAudience) || countries.length > 0 || hasAnyText(data.videoRate, data.collabRate, data.storyRate, data.storyPackRate, data.usageRate) || data.includes.length > 0) && <section className="tpCommercial" id="tp-rates">{(hasAnyText(data.followers, data.monthlyViews, data.womenAudience) || countries.length > 0) && <div className="tpMetrics"><small>AUDIENCIA</small><h2>Alcance y comunidad.</h2>{hasAnyText(data.followers, data.monthlyViews, data.womenAudience) && <div>{hasText(data.followers) && <span><b>{data.followers}</b>Seguidores</span>}{hasText(data.monthlyViews) && <span><b>{data.monthlyViews}</b>Vistas / mes</span>}{hasText(data.womenAudience) && <span><b>{data.womenAudience}</b>Mujeres</span>}</div>}{countries.length > 0 && <p>{countries.join(" · ")}</p>}</div>}{(hasAnyText(data.videoRate, data.collabRate, data.storyRate, data.storyPackRate, data.usageRate) || data.includes.length > 0) && <div className="tpRates"><small>TARIFAS BASE</small>{hasText(data.videoRate) && <div><span>Video UGC</span><b>{data.videoRate}</b></div>}{hasText(data.collabRate) && <div><span>Reel colaborativo</span><b>{data.collabRate}</b></div>}{hasText(data.storyRate) && <div><span>Historia con CTA</span><b>{data.storyRate}</b></div>}{hasText(data.storyPackRate) && <div><span>Pack de historias</span><b>{data.storyPackRate}</b></div>}{hasText(data.usageRate) && <div><span>Derechos de pauta / mes</span><b>{data.usageRate}</b></div>}{data.includes.length > 0 && <p>Incluye: {data.includes.join(" · ")}</p>}</div>}</section>}
    {hasAnyText(data.email, data.whatsapp) && <section className="tpContact"><div><span>BOOKING / COLLABORATIONS</span><h2>Disponible para tu próxima campaña.</h2></div><div>{hasText(data.email) && <a href={emailLink(data.email)} data-analytics-target="email">{data.email} ↗</a>}{hasText(data.whatsapp) && <a href={whatsappLink(data.whatsapp)} target="_blank" rel="noreferrer" data-analytics-target="whatsapp">{data.whatsapp}</a>}</div></section>}
